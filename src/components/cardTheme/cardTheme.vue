...
const getCardsFromCache = async () => {
  if (navigator.serviceWorker && navigator.serviceWorker.controller) {
    ...
    for (const request of cacheKeys) {
      ...
      if (requestUrl.pathname.startsWith('/cards/')) { // Vérifie bien que les cartes sont dans le bon cache
        try {
...
const DeleteCards = async (cardName: string) => {
  ...
  if ('caches' in window) {
    try {
      ...
      navigator.serviceWorker?.controller?.postMessage({ type: 'DELETE_CARD', card: cardName });
    } catch (error) {
  ...
  <section class="bg-white shadow-sm">
    ...
    <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
      <form @submit.prevent="CreateCards" class="mt-5 flex flex-col gap-4">
        <div>
          <label for="cardName" class="block text-sm font-medium text-gray-700">Nom de la
            carte</label>
          <input
            id="cardName"
            v-model="CardName"
